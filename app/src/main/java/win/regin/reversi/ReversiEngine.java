...
public class ReversiEngine extends Thread {

    private static final int BOARD_SIZE = 8;
    static final byte PLAYER_BLACK = 0;
    static final byte PLAYER_EMPTY = 1;
    static final byte PLAYER_WHITE = 2;
    static final int PLAYER_ZEBRA = 1;
    static final int INFINIT_TIME = 10000000;
    static final int
            MSG_ERROR = 0,
            MSG_BOARD = 1,
    ...
    public ReversiEngine(Context context, Handler handler) {
        mContext = context;
        mHandler = handler;
    }
    ...
    public void makeMove(Move move) throws InvalidMove, EngineError {
        ...
        if (mPlayerInfo[mSideToMove].skill == 0
                && getEngineState() == ReversiEngine.ES_PLAYINPROGRESS) {
            stopMove();
            waitForEngineState(ES_USER_INPUT_WAIT, 1000);
        }

        if (getEngineState() != ES_USER_INPUT_WAIT) {
            // Log.d("ReversiEngine", "Invalid Engine State");
            return;
        }
    ...
    private JSONObject Callback(int msgcode, JSONObject data) {
        ...
        Bundle b = new Bundle();
        msg.setData(b);
        // Log.d("ReversiEngine", String.format("Callback(%d,%s)", msgcode, data.toString()));
        if (bInCallback) {
    ...
    public boolean isThinking() {
        return getEngineState() == ReversiEngine.ES_PLAYINPROGRESS;
    }
    ...
    private native void zeSetAutoMakeMoves(int autoMakeMoves);

    private native void zeSetSlack(float slack);

    private native void zeSetPerturbation(float perturbation);

    private native void zeSetForcedOpening(String openingName);
