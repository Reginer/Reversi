...
import win.regin.reversi.ReversiEngine.InvalidMove;
...
public class BoardView extends View {
    ...
    private ReversiActivity mReversiActivity = null;
    ...
    private void initBoardView() {
        if (ReversiActivity.class.isInstance(getContext())) {
            setDroidZebra((ReversiActivity) getContext());
        }
    ...
    public ReversiActivity getDroidZebra() {
        return mReversiActivity;
    }

    public void setDroidZebra(ReversiActivity activity) {
        mReversiActivity = activity;
    }
    ...
    public Move getMoveFromCoord(float x, float y) throws InvalidMove {
        ...
        if (bx < 0 || bx >= ReversiActivity.BOARD_SIZE || by < 0 || by >= ReversiActivity.BOARD_SIZE) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i <= ReversiActivity.BOARD_SIZE; i++) {
            mPaint.setColor(mColorLine);
            canvas.drawLine(mBoardRect.left + i * mSizeCell, mBoardRect.top, mBoardRect.left + i * mSizeCell, mBoardRect.top + mSizeCell * ReversiActivity.BOARD_SIZE, mPaint);
            canvas.drawLine(mBoardRect.left, mBoardRect.top + i * mSizeCell, mBoardRect.left + mSizeCell * ReversiActivity.BOARD_SIZE, mBoardRect.top + i * mSizeCell, mPaint);
        }
        ...
        if (mMoveSelection != null) {
            if (mShowSelectionHelpers) {
                if (mReversiActivity.isValidMove(mMoveSelection)) {
        ...
        for (int i = 0; i < ReversiActivity.BOARD_SIZE; i++) {
            for (int j = 0; j < ReversiActivity.BOARD_SIZE; j++) {
                if (getDroidZebra().getBoard()[i][j].getState() == ReversiEngine.PLAYER_EMPTY) {
                    continue;
                }
                if (getDroidZebra().getBoard()[i][j].getState() == ReversiEngine.PLAYER_BLACK) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mSizeCell = Math.min(mSizeX / (ReversiActivity.BOARD_SIZE + 1), mSizeY / (ReversiActivity.BOARD_SIZE + 1));
        lineWidth = Math.max(1f, mSizeCell / 40f);
        gridCirclesRadius = Math.max(3f, mSizeCell / 13f);
        mBoardRect.set(
                mSizeX - mSizeCell / 2 - mSizeCell * ReversiActivity.BOARD_SIZE,
                mSizeY - mSizeCell / 2 - mSizeCell * ReversiActivity.BOARD_SIZE,
                mSizeX - mSizeCell / 2,
                mSizeY - mSizeCell / 2
        );
    ...
    private void updateSelection(int bX, int bY, boolean bMakeMove, boolean bShowSelection) {
        ...
        if (bX < 0 || bX >= ReversiActivity.BOARD_SIZE) {
            bX = mMoveSelection.getX();
        }

        if (bY < 0 || bY >= ReversiActivity.BOARD_SIZE) {
        ...
        if (bMakeMove) {
            ...
            if (getDroidZebra().mZebraThread.isValidMove(mMoveSelection)) {
                ...
                if (mReversiActivity.mZebraThread.isThinking() && !mReversiActivity.mZebraThread.isHumanToMove()) {
                    mReversiActivity.showBusyDialog();
                } else {
                    try {
