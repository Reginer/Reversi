...
#define JNIFn(Package, Class, Fname) JNICALL Java_win_regin_##Package##_##Class##_##Fname
...
#define DROIDZEBRA_JNI_THROW(msg) { _reversi_throw_engine_error(env, (msg)); return; }

char android_files_dir[256];

static void _reversi_undo_turn(int *side_to_move);

static void _reversi_redo_turn(int *side_to_move);

static void _reversi_on_settings_change(void);

static void _reversi_compute_evals(int side_to_move);

static void _reversi_throw_engine_error(JNIEnv *env, const char *msg);

// undo stack helpers
static void _reversi_undo_stack_push(int val);

static int _reversi_undo_stack_pop(void);

static void _reversi_undo_stack_clear(void);

static int _reversi_can_redo(void);
...
JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeGlobalInit)(JNIEnv *env, jobject thiz, jstring files_dir) {
    DROIDZEBRA_JNI_SETUP;

    char binbookpath[FILENAME_MAX];
    char cmpbookpath[FILENAME_MAX];
    time_t timer;
...
JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeGlobalTerminate)(JNIEnv *env, jobject thiz) {
    DROIDZEBRA_JNI_SETUP;
    global_terminate();
    DROIDZEBRA_JNI_CLEAN;
}
...
JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeForceReturn)(JNIEnv *env, jobject thiz) {
    force_return = 1;
}

JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeForceExit)(JNIEnv *env, jobject thiz) {
    force_return = 1;
    force_exit = 1;
}
...
JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeSetPlayerInfo)(JNIEnv *env, jobject thiz, jint _player, jint _skill,
...
JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeSetSlack)(JNIEnv *env, jobject thiz, jfloat slack) {
    s_slack = slack;
}

JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeSetAutoMakeMoves)(JNIEnv *env, jobject instance, jint autoMakeMoves) {
    auto_make_forced_moves = autoMakeMoves;
}

JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeSetPerturbation)(JNIEnv *env, jobject thiz, jfloat perturbation) {
    s_perturbation = perturbation;
    //set_perturbation( floor( s_perturbation * 128.0 ) );
}

JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeSetHumanOpenings)(JNIEnv *env, jobject instance, jint enable) {
    s_human_opening = enable;
    //toggle_human_openings(s_human_opening);
}

JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeSetPracticeMode)(JNIEnv *env, jobject instance, jint enable) {
    s_practice_mode = enable;
}

JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeSetUseBook)(JNIEnv *env, jobject instance, jint enable) {
    s_use_book = enable;
}

JNIEXPORT void
JNIFn(reversi, ReversiEngine, zeSetForcedOpening)(JNIEnv *env, jobject instance, jstring openingName_) {
    int i;
    const char *str = NULL;

    s_forced_opening_seq = NULL;
    if (!openingName_) return;
    str = (*env)->GetStringUTFChars(env, openingName_, NULL);
    if (!str) return;
    ...
    (*env)->ReleaseStringUTFChars(env, openingName_, str);
}

JNIEXPORT jboolean
JNIFn(reversi, ReversiEngine, zeGameInProgress)(JNIEnv *env, jobject thiz) {
    return (jboolean) (game_in_progress() ? JNI_TRUE : JNI_FALSE);
}

JNIEXPORT void
JNIFn(reversi, ReversiEngine, zePlay)(JNIEnv *env, jobject thiz, jint providedMoveCount,
            ...
            if (provided_move_index >= provided_move_count) {
                if (skill[side_to_move] == 0) {
                    ...
                    if (auto_make_forced_moves && move_count[disks_played] == 1) {
                        ...
                        // compute evaluations
                        if (s_practice_mode) {
                            _reversi_compute_evals(side_to_move);
                            if (force_exit) break;
                        ...
                        if (evt.type == UI_EVENT_EXIT) {
                            ...
                            _reversi_undo_stack_clear(); // once player makes the move undo info is stale
                        } else if (evt.type == UI_EVENT_UNDO) {
                            _reversi_undo_turn(&side_to_move);
                            // adjust for increment at the beginning of the game loop
                            if (side_to_move == BLACKSQ)
                                score_sheet_row--;
                            continue;
                        } else if (evt.type == UI_EVENT_REDO) {
                            _reversi_redo_turn(&side_to_move);
                            // adjust for increment at the beginning of the game loop
                            if (side_to_move == BLACKSQ)
                                score_sheet_row--;
                            continue;
                        } else if (evt.type == UI_EVENT_SETTINGS_CHANGE) {
                            _reversi_on_settings_change();
...
void _reversi_undo_turn(int *side_to_move) {
    int human_can_move = 0;
    int curr_move;

    if (score_sheet_row == 0 && *side_to_move == BLACKSQ) return;

    _reversi_undo_stack_push(disks_played);
...
void _reversi_redo_turn(int *side_to_move) {
    int target_disks_played = 0;
    int curr_move;

    if (!_reversi_can_redo()) return;

    target_disks_played = _reversi_undo_stack_pop();
...
void _reversi_on_settings_change(void) {
    set_slack((int) floor(s_slack * 128.0));
    set_perturbation((int) floor(s_perturbation * 128.0));
    toggle_human_openings(s_human_opening);
    set_forced_opening(s_forced_opening_seq);
}

void _reversi_compute_evals(int side_to_move) {
...
void _reversi_throw_engine_error(JNIEnv *env, const char *msg) {
    jclass exc = (*env)->FindClass(env, "win/regin/reversi/EngineError");
    if (exc) (*env)->ThrowNew(env, exc, msg);
}

void _reversi_undo_stack_push(int val) {
    assert(s_undo_stack_pointer < 64);
    s_undo_stack[s_undo_stack_pointer++] = val;
}

int _reversi_undo_stack_pop(void) {
    assert(s_undo_stack_pointer > 0);
    return s_undo_stack[--s_undo_stack_pointer];
}

void _reversi_undo_stack_clear(void) {
    s_undo_stack_pointer = 0;
}

int _reversi_can_redo(void) {
